...
public class AssetBundleManager : MonoBehaviour
{
    ...
    public void DownloadCancle()
    {
        ...
        //번들 소유 확인
        foreach (string bundleNameEach in bundleName)
        {
            HasBundle(bundleNameEach, (state) =>
            {
                Manager.Resource.ReleaseBundle();
                Caching.ClearAllCachedVersions(bundleNameEach);
            });
        }
    }
    ...
    /// 번들 다운로드 팝업
    /// </summary>
    public void DownLoadBundle(Action call, params string[] downloadFileName)
    {
        ...
        if (VersionTable == null)
        {
            ...
            {
                if (refeshState)
                    DownLoadPopUpProcess(downloadFileNameList, call, downloadFileName);
                else
    ...
    /// 다운로드 준비
    /// </summary>
    void DownLoadPopUpProcess(List<string> downloadFileNameList, Action call, string[] downloadFileName)
    {
        MakeDownloadList(downloadFileNameList, out List<string> downloadList);
        StartCoroutine(DownloadCoreRoutine(call, downloadList, downloadFileName));
    }
    ...
    /// </summary>
    IEnumerator DownloadCoreRoutine(Action call, List<string> downLoadArray, string[] downloadFileName)
    {
        ...
        if (waitQueue.Count > 0)
        {
            ...
            downloadCo = StartCoroutine(DownLoadProcessRoutine(waitQueue, downLoadArray.Count, fetchSize));
        }
        call?.Invoke();
    }
